import"./exercisesRightFirstPart-38f2a99f.js";const _=document.querySelector(".button-close"),q=document.querySelector(".btn-delete-favorites");document.querySelector(".btn-rating");const T=document.querySelector(".modal-card-container-f"),u=document.querySelector(".backdrop"),k=document.querySelector(".modal-window");let p;_.addEventListener("click",g);window.addEventListener("keydown",m);u.addEventListener("click",f);function m(e){e.code==="Escape"&&g()}function f(e){e.target===u&&g()}function g(){console.log("hi"),u.classList.add("is-hidden"),window.removeEventListener("keydown",m),u.removeEventListener("click",f)}function w(e){u.classList.remove("is-hidden"),console.log(" привіт це імпорт"),C(e);T.innerHTML=p,L()}function C(e){const i=localStorage.getItem("exerciseCard"),t=JSON.parse(i);return t.length?t.forEach(n=>{const{id:o,gifUrl:s,name:l,rating:a,target:r,bodyPart:h,equipment:y,popularity:x,burnedCalories:b,description:S}=n;if(o!=e){console.log("немає співпадіння");return}else return k.setAttribute("data-id",`${o}`),p=`      
                    ...
                    <div>
                        ...
                        <form action="rating__form">
                            ...
                            <div class="info">
                                ...
                                <p class="info-item">
                                    Body Part<span class="character">${h}</span>
                                </p>
                                <p class="info-item">
                                    Equipment<span class="character">${y}</span>
                                </p>
                                <p class="info-item">
                                    Popular<span class="character"> ${x}</span>
                                </p>
                                <p class="info-item">
    ...
    `}):console.error("No data found in local storage")}function L(){const e=document.querySelectorAll(".rating");e.length>0&&i();function i(){let t,n;for(let a=0;a<e.length;a++){const r=e[a];o(r)}function o(a){s(a),l()}function s(a){t=a.querySelector(".rating__active"),n=a.querySelector(".rating__value")}function l(a=n.innerHTML){const r=a/.05;t.style.width=`${r}%`}}}q.addEventListener("click",E);function E(e){const i=localStorage.getItem("exerciseCard");let t=JSON.parse(i);console.log(t);const n=e.target.closest(".modal-window").dataset.id;console.log(n),t.forEach((o,s)=>{if(console.log(s),o.id!=n){console.log("немає співпадіння");return}else t.length===1?t=[]:(t.splice(s,1),console.log("є співпадіння"),console.log(t)),localStorage.setItem("exerciseCard",JSON.stringify(t)),g();s=1}),v()}const $={onClickStart:w},M="https://your-energy.b.goit.study/api/quote",c={quote:document.querySelector(".quote_text"),authorTitle:document.querySelector(".author_name"),favoriteEx:document.querySelector(".favorite_exercises"),defaultText:document.querySelector(".default_text")};let d;A();async function A(){try{const e=await fetch(M);if(e.ok){const i=await e.json(),t=i.quote,n=i.author;c.quote.innerHTML=t,c.authorTitle.innerHTML=n}else c.quote.innerHTML="Не вдалося отримати цитату",authorTitle.innerHTML=""}catch(e){console.error(e),c.quote.innerHTML="Помилка при отриманні цитати",authorTitle.textContent=""}}v();function v(){const e=localStorage.getItem("exerciseCard"),i=JSON.parse(e);if(i.length===0)c.defaultText.style.display="block";else{c.defaultText.style.display="none";const t=document.querySelector(".exercises-list");i.forEach(n=>{const{id:o,name:s,target:l,bodyPart:a,burnedCalories:r}=n;d=document.createElement("li"),d.className="exercises-item",d.setAttribute("data-id",`${o}`),d.innerHTML=` <div class="exercise-navigation">
